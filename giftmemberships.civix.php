...
function _giftmemberships_civix_class_loader($class) {
  if ($class === 'CRM_Giftmemberships_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Giftmemberships_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Giftmemberships\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Giftmemberships\\') === 0) {
 ...
 */
function _giftmemberships_civix_civicrm_config($config = NULL) {
 ...
 */
function _giftmemberships_civix_civicrm_install() {
 ...
 */
function _giftmemberships_civix_civicrm_enable(): void {
  _giftmemberships_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
 ...
 * @return bool
 */
function _giftmemberships_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _giftmemberships_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _giftmemberships_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _giftmemberships_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _giftmemberships_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _giftmemberships_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _giftmemberships_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _giftmemberships_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
